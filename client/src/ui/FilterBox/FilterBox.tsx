...
const FilterBox = ({
    ...
    return (
        <div className="relative w-auto bg-midnight rounded-[10px] cursor-pointer">
            <div
                className="flex justify-between items-center mx-[15px] my-[9px]"
                onClick={() => toggleVisibility(!isVisible)}
            >
                <Image src={FilterIcon} height={13} width={17} alt="filter-icon"/>
                <p className="text-[14px] leading-[17px] text-snowWhite mx-[10px]">Filters</p>
            ...
            {isVisible && (
                <div className="absolute top-0 left-0 w-full h-auto bg-midnight rounded-[10px]">
                    <div
                        className="flex justify-between items-center mx-[15px] my-[9px] border-b border-b-oceanBlue pb-2.5" onClick={() => toggleVisibility(false)}>
                        <Image src={FilterIcon} height={13} width={17} alt="filter-icon"/>
                        <p className="text-[14px] leading-[17px] text-snowWhite mx-[10px]">Filters</p>
                        <Image src={ChevronIcon} height={9} width={9} alt="chevron-icon"/>
                    </div>
                    <div className="flex-col mx-[15px]">
                        ...
                        <div className="flex-col w-full mb-[15px] mt-[5px]">
                            <Checkbox label="Paid" currentFilter={statusFilter} setCurrentFilter={setStatusFilter}/>
                            <Checkbox label="Unpaid" currentFilter={statusFilter} setCurrentFilter={setStatusFilter}/>
                        </div>
                    </div>
                </div>
            )}
