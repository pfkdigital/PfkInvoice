...
const ClientTable = ({ headers, clients, showBar }: ClientTableProps) => {
  const renderHeaders = () => {
  ...
  const renderClientData = (data: ClientType[]) => {
    return data.map((item, index) => (
      <Link href={`/clients/${item.id}`} key={index}>
        <tr
          className={
            "w-full grid grid-cols-3 h-auto bg-eclipse rounded-[7px] px-[16px] py-[9px] mb-[10px] md:grid-cols-6 md:bg-midnight md:py-[12px]"
          }
        >
          <td
            className={
  ...
  return (
    <div className="w-full h-fit mt-[16px] rounded-[20px] md:p-5 md:bg-eclipse">
      {showBar && (
