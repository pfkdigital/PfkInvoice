...
const InvoiceTable = ({
  ...
  const renderInvoiceData = (data: InvoiceType[]) => {
    return data.map((invoice, index) => (
      <Link key={index} href={`/invoices/${invoice.id}`}>
        <tr
          className={`w-full h-auto grid grid-cols-4 ${
            lightRows ? "bg-midnight" : "bg-eclipse"
          } rounded-[7px] px-[16px] py-[9px] mb-[10px] md:grid-cols-8 md:bg-midnight md:py-[12px]`}
        >
  ...
  return (
    <div className="w-full h-fit mt-[16px] rounded-[20px] md:p-5 md:pt-5 md:bg-eclipse">
      {showBar && (
